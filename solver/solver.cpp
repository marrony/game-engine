...
int main(void) {
	...
	jacobians[1].m_linear = vector::make(0.0f, 0.5f, 0.5f);
	jacobians[0].m_constraint_force = 1.0f;
	jacobians[1].m_constraint_force = 1.2f;
...
void Solve3DVerbose(Jacobian *j, int num_jacobians, SimObject *o, int num_objects) {
	...
	for (int x = 0; x < num_jacobians; x++) {
		...
		Vector3 JM_1 = j[x].m_linear * o[j[x].m_object_index].m_invMass;
		...
		D[x] = vector::dot(JM_1, j[x].m_linear);
...
//		D_[x] = D[x] * j[x].m_mixing_coefficient;
	...
	for (int iter = 0; iter < NUM_SOLVER_ITERATIONS; iter++) {
		...
		for (int j_index = 0; j_index < num_jacobians; j_index++) {
			...
			printf("\t\t\taccel (object %d) = (%f %f %f)\n", j[j_index].m_object_index, a[j[j_index].m_object_index].x, a[j[j_index].m_object_index].y, a[j[j_index].m_object_index].z);
		}
	}
